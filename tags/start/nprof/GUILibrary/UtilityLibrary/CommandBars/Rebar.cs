...
namespace UtilityLibrary.CommandBars
{
	...
	public class ReBar : Control, IMessageFilter
	{
		...
		private void PaintBackground()
		{
			if ( needsColorUpdate)
			{
				...
				for ( int i = 0; i < bands.Count; i++ )
				{
					...
					VSNetToolBar toolBar = (VSNetToolBar)bands[i];
		...
		private void DrawGripper(Graphics g, Rectangle bounds)
		{
			...
			g.FillRectangle(new SolidBrush(ColorUtil.VSNetStripeColor), bounds);
		...
		void UpdateBand(int index)
		{
			...
			VSNetToolBar tb = (VSNetToolBar)bands[actualIndex];
		...
		REBARBANDINFO GetBandInfo(int index)
		{
			...
			if ( !IsCommonCtrl6() )
			{
				...
				rbbi.clrBack = (int)ColorUtil.RGB(ColorUtil.VSNetStripeColor.R,
					ColorUtil.VSNetStripeColor.G, ColorUtil.VSNetStripeColor.B);
			...
			VSNetToolBar tb = (VSNetToolBar)band;
