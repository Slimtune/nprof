...
namespace UtilityLibrary.CommandBars
{
	...
	public class ToolBarEx : System.Windows.Forms.Control, IChevron
	{
		...
		bool showingChevronMenu = false;
		
		// Bar Type --Standard or MenuBar
		...
		// Image List Support
		// -- not really used to draw the images in the ToolBarItem
		// but as a helper to size the ToolBarButtons
		ImageList imageListHelper = null;
		// The actual imageList that we associate with the ToolBar
		ImageList imageList = null;
				
		// Windows XP, using a manifest support
		bool bGotIsCommonCtrl6 = false;
		bool isCommonCtrl6 = false;

		// Menu bar tracking support
		...
		// Rebar interaction suport
		bool useNewRow = true;
		internal ReBar parentRebar = null;
				
		// Design Suppport
		internal ToolBarItem placeHolderToolBarItem = null;
		internal bool placeHolderAdded = false;
		#endregion
		...
		protected override CreateParams CreateParams
		{
			get
			{
				...
				if ( parentRebar != null )
				{
					createParams.Style |= (int)(ToolBarStyles.TBSTYLE_FLAT | ToolBarStyles.TBSTYLE_TRANSPARENT);
                }
		...
		protected override void OnHandleCreated(EventArgs e)
		{
			...
			if ( parentRebar != null && parentRebar.RebarDesigner != null && placeHolderAdded == false && items.Count == 0 )
			{
...
//			if ( barType == BarType.MenuBar)
			{
				if ((e.Button == MouseButtons.Left) && (e.Clicks == 1))
				{
					Point point = new Point(e.X, e.Y);
					int index = HitTest(point);
					if (IsValid(index) && items[index].Enabled)
					{
						TrackDropDown(index);
						return;
					}
		...
		protected override void WndProc(ref Message message)
		{
			...
			switch (message.Msg)
			{
				...
				case (int)Msg.WM_LBUTTONDOWN:
					...
					if ( parentRebar != null && parentRebar.Bands.Count == 0 && parentRebar.RebarDesigner != null )
					{
						...
						pt = PointToScreen(pt);
						parentRebar.PointToClient(pt);
						parentRebar.RebarDesigner.PassMsg(ref message);
					}
					break;
		...
		internal void AddPlaceHolderToolBarItem()
		{
			...
			button.idCommand = 0;
            WindowsAPI.SendMessage(Handle, ToolBarMessages.TB_INSERTBUTTON, 0, ref button);
	
			TBBUTTONINFO tbi = GetButtonInfo(0, item);
			WindowsAPI.SendMessage(Handle, ToolBarMessages.TB_SETBUTTONINFOW, 0, ref tbi);
		...
		void TrackDropDown(int index)
		{
			while ( index >= 0 )
			{
				...
				hookHandle = WindowsAPI.SetWindowsHookEx(WindowsHookCodes.WH_MSGFILTER, 
		...
		void NotifyNeedTextA(ref Message m)
		{
			...
			if ( toolTip != null && toolTip != string.Empty )
			{
				...
				if ( RightToLeft == RightToLeft.Yes ) ttt.flags |= ToolTipFlags.TTF_RTLREADING;
		...
		void NotifyCustomDrawToolBar(ref Message m)
		{
			...
			if ( parentRebar != null && parentRebar.Bands.Count == 0 || items.Count == 0) 
			{
			...
			int index = (int)tbcd.nmcd.dwItemSpec;
			...
			bool hot = (bool)((tbcd.nmcd.uItemState & (uint)CustomDrawItemStateFlags.CDIS_HOT) != 0);
			bool selected = (bool)((tbcd.nmcd.uItemState & (uint)CustomDrawItemStateFlags.CDIS_SELECTED) != 0);
			bool disabled = (bool)((tbcd.nmcd.uItemState & (uint)CustomDrawItemStateFlags.CDIS_DISABLED) != 0);
			...
			if ( item.Style == ToolBarItemStyle.DropDownButton ) 
			{
				DrawButtonArrowGlyph(g, rectangle, disabled);
			...
			Image image = null;
			if ( imageList != null && item.ImageListIndex != -1 )
			{
				if ( item.ImageListIndex < imageList.Images.Count )
					image = imageList.Images[item.ImageListIndex];
			}
			else
			{
				image = item.Image;
			...
			if ( hasText )
			{
				...
				Point pos;
				if ( barType == BarType.MenuBar || image == null)	
				{
				...
				Color brushTextColor = SystemColors.MenuText;
				if ( disabled )
					brushTextColor = Color.FromArgb(120, SystemColors.MenuText);
				using ( Brush b = new SolidBrush(brushTextColor) )
				{
					g.DrawString(item.Text, SystemInformation.MenuFont, b, pos, stringFormat); 
				}
			}
		...
		protected void DrawButtonArrowGlyph(Graphics g, Rectangle rectangle, bool disabled)
		{
			// Draw arrow glyph
			...
			Color brushColor = SystemColors.MenuText;
			if ( disabled )
				brushColor = Color.FromArgb(120, SystemColors.MenuText);
		...
		private MenuItem FindShortcutItem(MenuItemExCollection items, Keys keys)
		{
			MenuItem resultItem = null;
			foreach (MenuItemEx item in items )
			{
		...
		TBBUTTONINFO GetButtonInfo(int index, ToolBarItem currentItem)
		{
			ToolBarItem item = null;
			if ( currentItem != null )
				item = currentItem;
			...
			else if ( item.Text != null && item.Text != string.Empty )
			{
				tbi.fsStyle = (int)ToolBarButtonStyles.TBSTYLE_BUTTON;
				tbi.cx = MARGIN;
				if ( item.Image != null || imageList != null)
				{
					if ( imageList != null && item.ImageListIndex != -1 && item.ImageListIndex < imageList.Images.Count )
					{
						tbi.cx += (short)(imageList.ImageSize.Width + MARGIN);
					}
			...
			else if (item.Image != null || imageList != null)
				tbi.iImage = index;
		...
		void RealizeItems()
		{
			...
			for (int i = 0; i < items.Count; i++)
			{
				...
				TBBUTTON button = new TBBUTTON();
				button.idCommand = i;
				WindowsAPI.SendMessage(Handle, ToolBarMessages.TB_INSERTBUTTON, i, ref button);
	
				TBBUTTONINFO tbi = GetButtonInfo(i, null);
				WindowsAPI.SendMessage(Handle, ToolBarMessages.TB_SETBUTTONINFOW, i, ref tbi);
		...
		void UpdateComboBoxPosition(int index)
		{
			...
			// In case it was make invisible by the user minimizing the window
			if ( !Items[index].ComboBox.Visible )
				Items[index].ComboBox.Visible = true;
		...
		void UpdateImageList()
		{
			...
			if ( imageList == null )
				...
				imageListHelper.ImageSize = imageList.ImageSize;
			...
			if ( parentRebar != null && parentRebar.RebarDesigner != null && items.Count == 0 )
			{
				// If we are faking the ToolBarItem, add at least one image
				// so that the imageList has a size
		...
		internal Rectangle GetItemRect(int index)
		{
			RECT rect = new RECT();
			WindowsAPI.SendMessage(Handle, (int)ToolBarMessages.TB_GETRECT, index, ref rect);
		...
		void UpdateItem(int index)
		{
			...
			{
				TBBUTTONINFO tbi = GetButtonInfo(index, null);
				WindowsAPI.SendMessage(Handle, ToolBarMessages.TB_SETBUTTONINFOW, index, ref tbi);
				...
				if ( parentRebar.RebarDesigner != null )
				{
					UpdateSize();
					parentRebar.UpdateBands();
				}
