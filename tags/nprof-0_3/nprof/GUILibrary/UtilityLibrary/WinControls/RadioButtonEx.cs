...
namespace UtilityLibrary.WinControls
{
	...
	public class RadioButtonEx : System.Windows.Forms.RadioButton
	{
		...
		void DrawRadioButtonState(DrawState state)
		{
			...
			{
				circleRect = new Rectangle(rect.Left, rect.Top + (rect.Height-CIRCLE_DIAMETER)/2, 
					CIRCLE_DIAMETER, CIRCLE_DIAMETER);
			}
			else
			{
				circleRect = new Rectangle(rect.Right-CIRCLE_DIAMETER-1, rect.Top + (rect.Height-CIRCLE_DIAMETER)/2, 
					CIRCLE_DIAMETER, CIRCLE_DIAMETER);
			}
		...
		void DrawCircleGlyph(Graphics g, DrawState state)
		{
			...
			if ( RightToLeft == RightToLeft.No )
			{
				point1 = new Point(rc.Left + 4, rc.Top + rc.Top + (rc.Height-4)/2 + 2);
				point2 = new Point(point1.X + 4, point1.Y);
				point3 = new Point(rc.Left + 6, rc.Top + rc.Top + (rc.Height-4)/2);
				point4 = new Point(point3.X, point3.Y + 4);
			}
			else
			{
				point1 = new Point(rc.Right - 4, rc.Top + rc.Top + (rc.Height-4)/2 + 2);
				point2 = new Point(point1.X - 4, point1.Y);
				point3 = new Point(rc.Right - 6, rc.Top + rc.Top + (rc.Height-4)/2);
				point4 = new Point(point3.X, point3.Y + 4);
			}
