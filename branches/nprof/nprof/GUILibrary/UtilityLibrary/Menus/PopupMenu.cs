			...
			if (dc.Expansion)
			{
				...
				if (hotCommand)
				{
					...
					case VisualStyle.IDE:
						...
						using (Pen selectPen = new Pen(SystemColors.Highlight))
						{
							// Draw the selection area white, because we are going to use an alpha brush
							using (SolidBrush whiteBrush = new SolidBrush(Color.White))
								g.FillRectangle(whiteBrush, selectArea);

							using (SolidBrush selectBrush = new SolidBrush(Color.FromArgb(70, SystemColors.Highlight)))
							{
				...
				if (dc.Separator)
				{
					...
					else
					{
						...
						case VisualStyle.IDE:
							...
							g.FillRectangle(new SolidBrush(ColorUtil.VSNetStripeColor), imageCol);
				...
				else
				{
					...
					if (hotCommand && mc.ComboBox == null )
					{
						...
						case VisualStyle.IDE:
							...
							break;
						case VisualStyle.Plain:
							using (SolidBrush selectBrush = new SolidBrush(SystemColors.Highlight))
					...
					if (mc.Checked)
					{
						...
						case VisualStyle.IDE:
							...
							if (mc.Enabled)
								boxPen = new Pen(SystemColors.Highlight);
							else
