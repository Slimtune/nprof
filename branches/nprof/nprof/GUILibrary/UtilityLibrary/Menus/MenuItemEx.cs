...
namespace UtilityLibrary.Menus
{
	...
	public class MenuItemEx : MenuItem
	{
		...
		string shortcuttext = "";
		Image icon = null;
		Color iconTransparentColor = Color.FromArgb(0,128,128);
		...
		{
			Initialize(null, shortcut, handler, imageList, imageIndex);
		}
		...
		public MenuItemEx(string name, Image icon) : base(name)
		{
			Initialize(icon, Shortcut.None, null, null, -1);
		}

		private void Initialize(Image icon, Shortcut shortcut, EventHandler handler, ImageList list, int imageIndex)
		{
			OwnerDraw = true;
		...
		protected override void OnMeasureItem(MeasureItemEventArgs e)
		{
			...
			if (Shortcut != Shortcut.None) 
			{
				...
				else 
				{
					...
					else if ( Shortcut == Shortcut.ShiftDel )
					{
						text = "Shift+Del";
					}
					else 
		...
		{
			set { iconTransparentColor = value;}
		...
		void DrawBackground(Graphics g, Rectangle bounds, DrawItemState state, bool toplevel, bool hasicon, bool enabled)
		{
		...
		void DrawMenuText(Graphics g, Rectangle bounds, string text, string shortcut, bool enabled, bool toplevel, DrawItemState state )
		{
			...
			TextUtil.DrawText(g, text, SystemInformation.MenuFont, new Rectangle(x, y, textSize.Width, textSize.Height));
		...
		void OnEmptyHandler(object sender, EventArgs e)
		{
			// Just to help create an owner draw separtor
		}
		#endregion
