...
namespace UtilityLibrary.CommandBars
{
	...
	/// <summary>
	/// Summary description for VSNetToolBar.
	/// </summary>
	public class VSNetToolBar : System.Windows.Forms.Control, IChevron
	{
		...
		public VSNetToolBar(bool useNewRow)
		{
			InitializeToolBar();
			this.useNewRow = useNewRow;
		}

		public VSNetToolBar(BarType type)
		{
			barType = type;
			InitializeToolBar();
		}

		public VSNetToolBar(BarType type, bool useNewRow)
		{
			barType = type;
			InitializeToolBar();
			this.useNewRow = useNewRow;
		...
		void NotifyCustomDrawToolBar(ref Message m)
		{
			...
			if (item.Checked) 
			{
				if ( hot )
					...
					g.FillRectangle(new SolidBrush(ColorUtil.VSNetToggleColor), rectangle);
				g.DrawRectangle(new Pen(new SolidBrush(SystemColors.Highlight), 1), 
			...
			else if (selected) 
			{
				...
				else 
				{
					if ( barType == BarType.MenuBar)
					{
						g.FillRectangle(new SolidBrush(ColorUtil.VSNetStripeColor), rectangle);
				...
				if ( item.Style == ToolBarItemStyle.DropDownButton )
				{
					...
					if ( hMainWindow != IntPtr.Zero &&  mainHasFocus)
					{
						...
						if ( rectangle.Contains(clientPoint))
						{
							...
							g.DrawLine(new Pen(new SolidBrush(SystemColors.Highlight), 1), 
