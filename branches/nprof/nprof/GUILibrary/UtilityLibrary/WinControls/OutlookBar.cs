...
namespace UtilityLibrary.WinControls
{
	...
	[ToolboxItem(false)]
	[Designer(typeof(UtilityLibrary.Designers.OutlookBarBandDesigner))]
	[TypeConverter(typeof(UtilityLibrary.Designers.OutlookBarBandConverter))]
	...
	[ToolboxItem(false)]
	[TypeConverter(typeof(UtilityLibrary.Designers.OutlookBarItemConverter))]
	...
	[Designer(typeof(UtilityLibrary.Designers.OutlookBarDesigner))]
	public class OutlookBar : System.Windows.Forms.Control
	{
        ...
        #region Properties
		[DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
		[Editor(typeof(UtilityLibrary.Designers.OutlookBarBandEditor),typeof(UITypeEditor))]
		...
		void DrawArrowButtons(Graphics g)
		{
			...
			if ( flatArrowButtons )
			{ 
				// If using flat arrow buttons, make the rectangle
				// slightly smaller --looks better--
				upArrowRect = new Rectangle(upArrowRect.Left, upArrowRect.Top, SystemInformation.VerticalScrollBarWidth-1,
			...
			{
				if ( flatArrowButtons )
					DrawFlatArrowButton(g, upArrowRect, ArrowGlyph.Up, UpFlatArrowState);
				else
				{
					if ( upArrowPressed )
			...
			if ( last < bands[currentBandIndex].Items.Count -1 )
			{
				...
				if ( flatArrowButtons )
					DrawFlatArrowButton(g, downArrowRect, ArrowGlyph.Down, DownFlatArrowState);	
				else
				{
					if ( downArrowPressed )
		...
		void DrawFlatArrowButton(Graphics g, Rectangle rc, ArrowGlyph arrowGlyph, DrawState state)
		{
			if ( HasChild() )
			{
			...
			using ( Pen p = new Pen(ColorUtil.VSNetBorderColor) )
			{
				...
				GDIUtil.DrawArrowGlyph(g, rc, 7, 4, arrowGlyph, Brushes.Black);
			}
		...
		void DrawArrowButton(Rectangle buttonRect, ButtonState state)
		{
			...
			if ( flatArrowButtons && !HasChild() )
			{
				...
				if ( arrowGlyph == ArrowGlyph.Up && first == 0 ) 
				{
					// up arrow is not visible no need to pain it
					return;
				}
				else if ( arrowGlyph == ArrowGlyph.Down && last == bands[currentBandIndex].Items.Count -1)
				{
				...
				if ( buttonRect.Contains(pos) )
				{
					...
					DrawFlatArrowButton(g, buttonRect, arrowGlyph, DrawState.Hot);
				}
				else
				{
					if ( arrowGlyph == ArrowGlyph.Up )
					...
					DrawFlatArrowButton(g, buttonRect, arrowGlyph, DrawState.Normal);
				}
			}
			else 
			...
			if ( To > From)
			{
				for (int y = rectTo.Top - headerHeight; y > rectFrom.Bottom; y -= headerHeight)
				{
					...
					WindowsAPI.BitBlt(hDC, viewPortRect.Left ,rectFrom.Bottom + 1, 
						viewPortRect.Width, y - rectFrom.Bottom - 1, hDCFrom, 0 , 0, PatBltTypes.SRCCOPY);

					// Draw To Bitmap
					WindowsAPI.BitBlt(hDC, viewPortRect.Left, y, viewPortRect.Width, 
						viewPortRect.Bottom - y + headerHeight, hDCTo, 0, 0, PatBltTypes.SRCCOPY);
			...
			else
			{
				...
				for (int y = rectFrom.Top + 1; y < rcTo.Top - headerHeight; y += headerHeight)
				{
					...
					WindowsAPI.BitBlt(hDC, viewPortRect.Left, rectFrom.Top,  viewPortRect.Width, 
						y - rectFrom.Top - 1, hDCTo, 0, 0, PatBltTypes.SRCCOPY);
										
					// Draw From bitmap
					WindowsAPI.BitBlt(hDC, viewPortRect.Left , y, 
						viewPortRect.Width, viewPortRect.Bottom - y, hDCFrom, 0 , 0, PatBltTypes.SRCCOPY);
					Thread.Sleep(animationSpeed);
