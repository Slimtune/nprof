...
public: 
	...
	void WalkStack() {
		...
		for(map<DWORD,ThreadID>::iterator i=threadMap.begin();i!=threadMap.end();i++) {
			...
			if(CompareFileTime(&userTime,&t)>0)
			{
				//else
				//{
				//	cout << userTime.dwLowDateTime - t.dwLowDateTime << "\n";// (((__int64)userTime.dwHighDateTime << 32) + userTime.dwLowDateTime - ((__int64)t.dwHighDateTime << 32) + t.dwLowDateTime) << "\n";
				//	cout << userTime.dwHighDateTime - t.dwHighDateTime << "\n";
				//}
				...
				while(true)
				{
					...
					while(StackWalk64(IMAGE_FILE_MACHINE_I386,GetCurrentProcess(),GetCurrentThread(),
						&sf64,&context,0,SymFunctionTableAccess64,SymGetModuleBase64,0))
					{
	...
	//ss >> val;
							...
							//DebugBreak();
							//cout << pSymbol->Name;
							/*functions->push_back(pSymbol->Address);*/
...
//	else
//	{
//		//Profiler* profiler=(Profiler*)clientData;
//		//profiler->FindManagedFrame(
//	}
//	return S_OK;
//}
...
//	static UINT timer;
//	map<DWORD,DWORD> switchMap;
//	bool FindManagedFrame(CONTEXT* context,HANDLE thread,vector<FunctionID>* functions)
//	{
//		bool found=false;
//		int count=0;
//		STACKFRAME64 sf64;
...
//			if(SUCCEEDED(profilerInfo->GetFunctionFromIP((LPCBYTE)sf64.AddrPC.Offset,&functionId)))
//			{
//				return true;
//			}
//
...
//					DebugBreak();
//					functions->push_back(pSymbol->Address);
//					const map< FunctionID, FunctionInfo* >::iterator found = signatures.find(pSymbol->Address);
//					if(found == signatures.end()){
//						FunctionInfo* function=new FunctionInfo();
//						function->signature=pSymbol->Name;
//						function->nameSpace="";
//
//						signatures.insert(make_pair(pSymbol->Address,function));
//					}
//
//				}
//			}
...
//				THREAD_SUSPEND_RESUME|THREAD_QUERY_INFORMATION|THREAD_GET_CONTEXT,
//				false,
//				threadId
//			);
//			//DebugBreak();
...
//
//
//			if(FindManagedFrame(&context,threadHandle,functions))
//			{
//				profilerInfo->DoStackSnapshot(
//					id,
//					StackWalk,
