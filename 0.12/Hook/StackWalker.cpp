...
BOOL StackWalker::MoveUpCallStack(CComPtr< ICorProfilerInfo2 > profilerInfo,HANDLE hThread, CONTEXT *context,BYTE** instructionPointer, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
...
#error "Platform not supported!"
  ...
  *instructionPointer=(BYTE*)s.AddrPC.Offset;
...
int StackWalker::ShowCallstack(HANDLE hThread, const CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
  CONTEXT c;;
  CallstackEntry csEntry;
  IMAGEHLP_SYMBOL64 *pSym = NULL;
...
#error "Platform not supported!"
  ...
  for (frameNum = 0; ; ++frameNum )
  {
    ...
    if (frameNum == 0)
      et = firstEntry;
    int id=this->OnCallstackEntry(et, csEntry);
	if(id!=0) {
		return 17;
	}
    ...
    if (s.AddrReturn.Offset == 0)
    {
      int id=this->OnCallstackEntry(lastEntry, csEntry);
	  if(id!=0) {
		  return 17;
	  }
      SetLastError(ERROR_SUCCESS);
...
int StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry)
{
  CHAR buffer[STACKWALK_MAX_NAMELEN];
