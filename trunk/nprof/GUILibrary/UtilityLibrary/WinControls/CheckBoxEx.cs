...
namespace UtilityLibrary.WinControls
{
	...
	public class CheckBoxEx : System.Windows.Forms.CheckBox
	{
		...
		void DrawCheckBoxState(DrawState state)
		{
			...
			Rectangle checkMark = Rectangle.Empty;
			if ( RightToLeft == RightToLeft.No )
			{
				checkMark = new Rectangle(rect.Left, rect.Top + (rect.Height-CHECK_MARK_SIZE)/2, 
					CHECK_MARK_SIZE, CHECK_MARK_SIZE);
			}
			else
			{
				checkMark = new Rectangle(rect.Right-CHECK_MARK_SIZE, rect.Top + (rect.Height-CHECK_MARK_SIZE)/2, 
					CHECK_MARK_SIZE, CHECK_MARK_SIZE);
			}
