...
{
	/// <summary>
	/// Summary description for VSNetMenuItem.
	/// </summary>
	public class VSNetMenuItem : MenuItem
	{
		...
		// constructors
		// ---------------------------------------------------
		public VSNetMenuItem() : this(null, null)
		{
		}	

		public VSNetMenuItem(string name, EventHandler handler, Shortcut shortcut) : this(name, handler)
		{
			Initialize(null, shortcut, handler, null, -1);
		}
		
		public VSNetMenuItem(string name, Bitmap icon, Shortcut shortcut, EventHandler handler) : this(name, handler)
		{
			Initialize(icon, shortcut, handler, null, -1);
		}

		public VSNetMenuItem(string name, ImageList imageList, int imageIndex, Shortcut shortcut, EventHandler handler) : this(name, handler)
		{
			Initialize(icon, shortcut, handler, imageList, imageIndex);
		}

		public VSNetMenuItem(string name, EventHandler handler) : base(name, handler)
		{
		...
		static public MenuItem CloneMenu(VSNetMenuItem currentItem)
		{
			VSNetMenuItem clonedItem = new VSNetMenuItem(currentItem.Text, (Bitmap)currentItem.Icon, 
			...
			foreach (VSNetMenuItem item in currentItem.MenuItems)
			{
		...
		protected override void OnSelect(EventArgs e)
		{
			...
			{
				if (parent is VSNetMenuItem)
					parent = (parent as VSNetMenuItem).Parent;
		...
		protected override void OnDrawItem(DrawItemEventArgs e)
		{
			...
			else if ( imageList != null && imageIndex != -1 )
			{
				...
				if (Checked && !hasIcon)
				DrawMenuGlyph(g, bounds, true);
			else if ( RadioCheck )
				DrawMenuGlyph(g, bounds, false);
		...
		public void DrawMenuGlyph(Graphics g, Rectangle bounds, bool bCheckMark)
		{
